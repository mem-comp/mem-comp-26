...
TESTKEY_GLOBAL_BUDGET = float(os.getenv('GLOBAL_BUDGET'))
TESTKEY_INSTANCE_BUDGET = float(os.getenv('INSTANCE_BUDGET'))
...
async def litellm_create_user(user_id, user_alias, budget):
...
async def litellm_create_key(user_id, key_alias, budget):
    async with httpx.AsyncClient() as client:
        resp = await client.post(
...
async def test_key_create(request):
    await litellm_create_user('test_user', 'test_user', TESTKEY_GLOBAL_BUDGET)
    res = await litellm_create_key('test_user', 'test_key', TESTKEY_INSTANCE_BUDGET)
    print(res)
    return text(f'ok\n\nkey: {res["key"]}\n')
...
async def harness_create_user(request):
    ...
    quota = float(request.args.get('quota'))
    await litellm_create_user(user_id, user_alias, quota)
...
async def harness_create_key(request):
    ...
    quota = float(request.args.get('quota'))
    res = await litellm_create_key(user_id, key_alias, quota)
...
async def harness_health(request):
    return json({'error': None, 'compat': ['v1']})
